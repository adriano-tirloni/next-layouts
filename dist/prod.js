...
var NextLayouts = function NextLayouts(PageComponent) {
  var props = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

  function layoutMap(ComponentFunction, ComponentIsThePage) {
    if (ComponentIsThePage && !ComponentFunction.layout) return Array(); //If there isn't a Layout String return the Layout

    if (!ComponentFunction.layout) return Array(ComponentFunction); //If there is a Layout string, split it and map it recursively 

    var map = ComponentFunction.layout.split('.').flatMap( //or layout => layoutMap(require(`./components/${layout}`).default)
    function (layout) {
      return layoutMap(NextLayouts.layoutsList[layout], false);
  ...
  return layoutMap(PageComponent, true).reduceRight(function (prevComponent, LayoutTree) {
    return react.createElement(LayoutTree, null, prevComponent); //or <LayoutTree>{prevComponent}</LayoutTree>
  }, react.createElement(PageComponent, props)); //or <PageComponent {...props} />
};

NextLayouts.layoutsList = {};

exports.default = NextLayouts;
